...
<!--
  需操作：对每个 `specs/[###-feature]/` 目录，至少说明其依赖的 `docus/` 文件或章节；
  若本结构文档本身约束某个需求，也要在相应文档中回指该 spec 目录。
-->
...
<!--
  ...
  额外要求：
  - 与具体需求相关的 `docus/` 文档，应标注对应 `specs/` 目录或需求编号。
  - 若属于全局规则，应明确写“全局适用”，避免伪装成单需求文档。
-->
```text
docus/ 依据来源文档
|── workflows
...
<!--
  ...
  额外要求：实现文件或关键函数的自描述必须注明其依据的 `docus/` 文件或章节；
  若实现属于单一需求，建议同时注明对应 `specs/` 目录。
-->
