...
<!-- 强制：与具体需求相关的文档必须回指对应 `specs/` 目录；相关 `src/` 实现文件必须再回指本文件。 -->
...
- **双向引用**：对应哪些 `specs/` 需求目录？相关 `src/` 文件/函数应如何回指本文件？
<!--例如：
- 输入：
...
- 双向引用：
  - 需求：`specs/003-outlier-preprocess/`
  - 实现回指：`src/tools/outlier_detector_2.py::run_step2` 头部说明引用本文件第 2 节与 `docus/workflows/workflow.md#Step 2`
-->
...
- 需要补充“本规范关联的 `specs/` 目录/需求编号”以及“被哪些 `src/` 文件落实”。
