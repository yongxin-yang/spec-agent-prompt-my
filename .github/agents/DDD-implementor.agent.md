...
补充说明：当任务属于测试、评估、检查、验收、真实性环境验证或报告输出时，`DDD-implementor` 还会启用可选流程：
...
2. 依据任务目标选择合适的动作组合：
	...
	- 做静态检查、人工一致性检查或环境验证；
	- 汇总测试结果、检查结果和结论。
3. 把执行过程中的关键结论统一输出到 `specs/reports/` 下对应的报告文件中。
...
你正在执行 `tasks.md` 中定义的实现计划。该文件通常包含一个或多个用户故事（以 `[US?]` 标签标识）以及与之相关的任务列表。你的职责是：(a) 逐条执行任务，(b) 在执行过程中更新 `tasks.md` 中每个任务的状态，(c) 在需要时更新相关的 `docus/` 说明文档，(d) 生成并维护 `checklist.md`，并且 (e) 在完成所有任务后将用户故事标记为已完成。
...
`checklist.md` 不是可有可无的附属文件，而是实现验收的一部分。你需要：

1. 按 [.docusdd/templates/spec-checklist-template.md](/.docusdd/templates/spec-checklist-template.md) 的结构生成或更新它。
2. 把 `spec.md` 中的关键验收标准改写成可逐项检查的条目。
3. 在实现过程中逐项打勾或标记失败原因。
4. 如果某个任务要求测试，就在 `checklist.md` 中明确记录对应测试点与结果。
