...
class PreProcessor:
    ...
    def _apply_evaluate_repetition(
        ...
        '''1. 你看这里，私有方法给写成公有方法，而且这么长的代码没有注释我人看不下去啊'''
        ...
        '''
        我的妈呀，你看你写的代码，还得独立出来一个_resolve_step03_artifact_context函数.而且还把json数据层层提取，
        仿佛只要两个存储数据结构对象中有一个键值对是不同的，就要重新用json构造方法把所有的键值对构造一遍。
        数据结构没有用到继承，而且语法上也没有进行重叠部分的简化
        尽量使用循环语句或者字典解包语法来进行相似json的构造
        student2 = {
            k: student1[k]
            for k in ("department", "name", "age")
        }
        或
        student2 = {
            **student1,
